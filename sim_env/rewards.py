...
# <<<<<
# >>>>> reward functions
def jbaek_reward_fun(env, action_n):
	...
	for i,n in enumerate(env.nodes):
		...
		for k in range(n.max_k):
			...
			if obs[k] == 1 and wks[k] > 0: # will only be completed if scheduled [TODO@luis - move or remove wks?]
				D_ik = 0; Dt_ik = 0
				...
				arr += np.ceil(Dt_ik*env.case["arrivals"][k])
...
def sr_or_reward(env, action_n):
	# a reward that seeks to maximize the instant success and overflow rate
	info = env.saved_step_info[2]
	rw = 0.0
	total_tasks = len(info["delay_list"])+info["overflow"]+info["discarded"]
	if total_tasks > 0:
		rw = (2.0*len(info["delay_list"])/total_tasks) - (0.5*info["overflow"]/total_tasks)
	return rw